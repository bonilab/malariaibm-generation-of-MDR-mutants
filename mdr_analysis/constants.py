# Defined by MaSim in C++
ENCODINGDB = [
    'KNY--C1x', 'KNY--C1X', 'KNY--C2x', 'KNY--C2X', 'KNY--Y1x', 'KNY--Y1X',
    'KNY--Y2x', 'KNY--Y2X', 'KYY--C1x', 'KYY--C1X', 'KYY--C2x', 'KYY--C2X',
    'KYY--Y1x', 'KYY--Y1X', 'KYY--Y2x', 'KYY--Y2X', 'KNF--C1x', 'KNF--C1X',
    'KNF--C2x', 'KNF--C2X', 'KNF--Y1x', 'KNF--Y1X', 'KNF--Y2x', 'KNF--Y2X',
    'KYF--C1x', 'KYF--C1X', 'KYF--C2x', 'KYF--C2X', 'KYF--Y1x', 'KYF--Y1X',
    'KYF--Y2x', 'KYF--Y2X', 'KNYNYC1x', 'KNYNYC1X', 'KNYNYC2x', 'KNYNYC2X',
    'KNYNYY1x', 'KNYNYY1X', 'KNYNYY2x', 'KNYNYY2X', 'KYYYYC1x', 'KYYYYC1X',
    'KYYYYC2x', 'KYYYYC2X', 'KYYYYY1x', 'KYYYYY1X', 'KYYYYY2x', 'KYYYYY2X',
    'KNFNFC1x', 'KNFNFC1X', 'KNFNFC2x', 'KNFNFC2X', 'KNFNFY1x', 'KNFNFY1X',
    'KNFNFY2x', 'KNFNFY2X', 'KYFYFC1x', 'KYFYFC1X', 'KYFYFC2x', 'KYFYFC2X',
    'KYFYFY1x', 'KYFYFY1X', 'KYFYFY2x', 'KYFYFY2X', 'TNY--C1x', 'TNY--C1X',
    'TNY--C2x', 'TNY--C2X', 'TNY--Y1x', 'TNY--Y1X', 'TNY--Y2x', 'TNY--Y2X',
    'TYY--C1x', 'TYY--C1X', 'TYY--C2x', 'TYY--C2X', 'TYY--Y1x', 'TYY--Y1X',
    'TYY--Y2x', 'TYY--Y2X', 'TNF--C1x', 'TNF--C1X', 'TNF--C2x', 'TNF--C2X',
    'TNF--Y1x', 'TNF--Y1X', 'TNF--Y2x', 'TNF--Y2X', 'TYF--C1x', 'TYF--C1X',
    'TYF--C2x', 'TYF--C2X', 'TYF--Y1x', 'TYF--Y1X', 'TYF--Y2x', 'TYF--Y2X',
    'TNYNYC1x', 'TNYNYC1X', 'TNYNYC2x', 'TNYNYC2X', 'TNYNYY1x', 'TNYNYY1X',
    'TNYNYY2x', 'TNYNYY2X', 'TYYYYC1x', 'TYYYYC1X', 'TYYYYC2x', 'TYYYYC2X',
    'TYYYYY1x', 'TYYYYY1X', 'TYYYYY2x', 'TYYYYY2X', 'TNFNFC1x', 'TNFNFC1X',
    'TNFNFC2x', 'TNFNFC2X', 'TNFNFY1x', 'TNFNFY1X', 'TNFNFY2x', 'TNFNFY2X',
    'TYFYFC1x', 'TYFYFC1X', 'TYFYFC2x', 'TYFYFC2X', 'TYFYFY1x', 'TYFYFY1X',
    'TYFYFY2x', 'TYFYFY2X'
]

# For monthly output files
# Defined by MaSim in C++
HEADER_NAME = [
    'time_elapsed', 'system_time', 'year', 'month', 'day', 'seasonal_factor', 
    'treatment_coverage_0_1', 'treatment_coverage_0_10', 'population', 'sep1', 
    'eir', 'sep2', 'bsp_2_10', 'bsp_0_5', 'blood_slide_prevalence', 'sep3', 
    'monthly_new_infection', 'sep4', 'monthly_new_treatment', 'sep5', 
    'monthly_clinical_episode', 'sep6', 'monthly_ntf_raw', 'sep7', 
    'KNY--C1x', 'KNY--C1X', 'KNY--C2x', 'KNY--C2X', 'KNY--Y1x', 'KNY--Y1X',
    'KNY--Y2x', 'KNY--Y2X', 'KYY--C1x', 'KYY--C1X', 'KYY--C2x', 'KYY--C2X',
    'KYY--Y1x', 'KYY--Y1X', 'KYY--Y2x', 'KYY--Y2X', 'KNF--C1x', 'KNF--C1X',
    'KNF--C2x', 'KNF--C2X', 'KNF--Y1x', 'KNF--Y1X', 'KNF--Y2x', 'KNF--Y2X',
    'KYF--C1x', 'KYF--C1X', 'KYF--C2x', 'KYF--C2X', 'KYF--Y1x', 'KYF--Y1X',
    'KYF--Y2x', 'KYF--Y2X', 'KNYNYC1x', 'KNYNYC1X', 'KNYNYC2x', 'KNYNYC2X',
    'KNYNYY1x', 'KNYNYY1X', 'KNYNYY2x', 'KNYNYY2X', 'KYYYYC1x', 'KYYYYC1X',
    'KYYYYC2x', 'KYYYYC2X', 'KYYYYY1x', 'KYYYYY1X', 'KYYYYY2x', 'KYYYYY2X',
    'KNFNFC1x', 'KNFNFC1X', 'KNFNFC2x', 'KNFNFC2X', 'KNFNFY1x', 'KNFNFY1X',
    'KNFNFY2x', 'KNFNFY2X', 'KYFYFC1x', 'KYFYFC1X', 'KYFYFC2x', 'KYFYFC2X',
    'KYFYFY1x', 'KYFYFY1X', 'KYFYFY2x', 'KYFYFY2X', 'TNY--C1x', 'TNY--C1X',
    'TNY--C2x', 'TNY--C2X', 'TNY--Y1x', 'TNY--Y1X', 'TNY--Y2x', 'TNY--Y2X',
    'TYY--C1x', 'TYY--C1X', 'TYY--C2x', 'TYY--C2X', 'TYY--Y1x', 'TYY--Y1X',
    'TYY--Y2x', 'TYY--Y2X', 'TNF--C1x', 'TNF--C1X', 'TNF--C2x', 'TNF--C2X',
    'TNF--Y1x', 'TNF--Y1X', 'TNF--Y2x', 'TNF--Y2X', 'TYF--C1x', 'TYF--C1X',
    'TYF--C2x', 'TYF--C2X', 'TYF--Y1x', 'TYF--Y1X', 'TYF--Y2x', 'TYF--Y2X',
    'TNYNYC1x', 'TNYNYC1X', 'TNYNYC2x', 'TNYNYC2X', 'TNYNYY1x', 'TNYNYY1X',
    'TNYNYY2x', 'TNYNYY2X', 'TYYYYC1x', 'TYYYYC1X', 'TYYYYC2x', 'TYYYYC2X',
    'TYYYYY1x', 'TYYYYY1X', 'TYYYYY2x', 'TYYYYY2X', 'TNFNFC1x', 'TNFNFC1X',
    'TNFNFC2x', 'TNFNFC2X', 'TNFNFY1x', 'TNFNFY1X', 'TNFNFY2x', 'TNFNFY2X',
    'TYFYFC1x', 'TYFYFC1X', 'TYFYFC2x', 'TYFYFC2X', 'TYFYFY1x', 'TYFYFY1X',
    'TYFYFY2x', 'TYFYFY2X', 'sep8', 
    'n_KNY--C1x', 'n_KNY--C1X', 'n_KNY--C2x', 'n_KNY--C2X', 'n_KNY--Y1x',
    'n_KNY--Y1X', 'n_KNY--Y2x', 'n_KNY--Y2X', 'n_KYY--C1x', 'n_KYY--C1X',
    'n_KYY--C2x', 'n_KYY--C2X', 'n_KYY--Y1x', 'n_KYY--Y1X', 'n_KYY--Y2x',
    'n_KYY--Y2X', 'n_KNF--C1x', 'n_KNF--C1X', 'n_KNF--C2x', 'n_KNF--C2X',
    'n_KNF--Y1x', 'n_KNF--Y1X', 'n_KNF--Y2x', 'n_KNF--Y2X', 'n_KYF--C1x',
    'n_KYF--C1X', 'n_KYF--C2x', 'n_KYF--C2X', 'n_KYF--Y1x', 'n_KYF--Y1X',
    'n_KYF--Y2x', 'n_KYF--Y2X', 'n_KNYNYC1x', 'n_KNYNYC1X', 'n_KNYNYC2x',
    'n_KNYNYC2X', 'n_KNYNYY1x', 'n_KNYNYY1X', 'n_KNYNYY2x', 'n_KNYNYY2X',
    'n_KYYYYC1x', 'n_KYYYYC1X', 'n_KYYYYC2x', 'n_KYYYYC2X', 'n_KYYYYY1x',
    'n_KYYYYY1X', 'n_KYYYYY2x', 'n_KYYYYY2X', 'n_KNFNFC1x', 'n_KNFNFC1X',
    'n_KNFNFC2x', 'n_KNFNFC2X', 'n_KNFNFY1x', 'n_KNFNFY1X', 'n_KNFNFY2x',
    'n_KNFNFY2X', 'n_KYFYFC1x', 'n_KYFYFC1X', 'n_KYFYFC2x', 'n_KYFYFC2X', 
    'n_KYFYFY1x', 'n_KYFYFY1X', 'n_KYFYFY2x', 'n_KYFYFY2X', 'n_TNY--C1x',
    'n_TNY--C1X', 'n_TNY--C2x', 'n_TNY--C2X', 'n_TNY--Y1x', 'n_TNY--Y1X',
    'n_TNY--Y2x', 'n_TNY--Y2X', 'n_TYY--C1x', 'n_TYY--C1X', 'n_TYY--C2x',
    'n_TYY--C2X', 'n_TYY--Y1x', 'n_TYY--Y1X', 'n_TYY--Y2x', 'n_TYY--Y2X',
    'n_TNF--C1x', 'n_TNF--C1X', 'n_TNF--C2x', 'n_TNF--C2X', 'n_TNF--Y1x',
    'n_TNF--Y1X', 'n_TNF--Y2x', 'n_TNF--Y2X', 'n_TYF--C1x', 'n_TYF--C1X',
    'n_TYF--C2x', 'n_TYF--C2X', 'n_TYF--Y1x', 'n_TYF--Y1X', 'n_TYF--Y2x',
    'n_TYF--Y2X', 'n_TNYNYC1x', 'n_TNYNYC1X', 'n_TNYNYC2x', 'n_TNYNYC2X',
    'n_TNYNYY1x', 'n_TNYNYY1X', 'n_TNYNYY2x', 'n_TNYNYY2X', 'n_TYYYYC1x',
    'n_TYYYYC1X', 'n_TYYYYC2x', 'n_TYYYYC2X', 'n_TYYYYY1x', 'n_TYYYYY1X',
    'n_TYYYYY2x', 'n_TYYYYY2X', 'n_TNFNFC1x', 'n_TNFNFC1X', 'n_TNFNFC2x',
    'n_TNFNFC2X', 'n_TNFNFY1x', 'n_TNFNFY1X', 'n_TNFNFY2x', 'n_TNFNFY2X',
    'n_TYFYFC1x', 'n_TYFYFC1X', 'n_TYFYFC2x', 'n_TYFYFC2X', 'n_TYFYFY1x',
    'n_TYFYFY1X', 'n_TYFYFY2x', 'n_TYFYFY2X', 'sep9', 'total_count'
]

# For summary output files
# Defined by MaSim in C++
SUMMARY_FILE_HEADER_NAME = [
  'seed', 'num_of_locations', 'beta', 'pop_size',
  'EIR_PfPR', 'sep1', 'bsp_2_10', 'bsp_0_5',
  'bsp', 'sep2', 'last_strategy_id', 'ntf'
]

# Defined by MaSim in C++
REPORTDAYS = [
    0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335, 366, 397, 425, 456,
    486, 517, 547, 578, 609, 639, 670, 700, 731, 762, 790, 821, 851, 882, 912,
    943, 974, 1004, 1035, 1065, 1096, 1127, 1155, 1186, 1216, 1247, 1277, 1308,
    1339, 1369, 1400, 1430, 1461, 1492, 1521, 1552, 1582, 1613, 1643, 1674,
    1705, 1735, 1766, 1796, 1827, 1858, 1886, 1917, 1947, 1978, 2008, 2039,
    2070, 2100, 2131, 2161, 2192, 2223, 2251, 2282, 2312, 2343, 2373, 2404,
    2435, 2465, 2496, 2526, 2557, 2588, 2616, 2647, 2677, 2708, 2738, 2769,
    2800, 2830, 2861, 2891, 2922, 2953, 2982, 3013, 3043, 3074, 3104, 3135,
    3166, 3196, 3227, 3257, 3288, 3319, 3347, 3378, 3408, 3439, 3469, 3500,
    3531, 3561, 3592, 3622, 3653, 3684, 3712, 3743, 3773, 3804, 3834, 3865,
    3896, 3926, 3957, 3987, 4018, 4049, 4077, 4108, 4138, 4169, 4199, 4230,
    4261, 4291, 4322, 4352, 4383, 4414, 4443, 4474, 4504, 4535, 4565, 4596,
    4627, 4657, 4688, 4718, 4749, 4780, 4808, 4839, 4869, 4900, 4930, 4961,
    4992, 5022, 5053, 5083, 5114, 5145, 5173, 5204, 5234, 5265, 5295, 5326,
    5357, 5387, 5418, 5448, 5479, 5510, 5538, 5569, 5599, 5630, 5660, 5691,
    5722, 5752, 5783, 5813, 5844, 5875, 5904, 5935, 5965, 5996, 6026, 6057,
    6088, 6118, 6149, 6179, 6210, 6241, 6269, 6300, 6330, 6361, 6391, 6422,
    6453, 6483, 6514, 6544, 6575, 6606, 6634, 6665, 6695, 6726, 6756, 6787,
    6818, 6848, 6879, 6909, 6940, 6971, 6999, 7030, 7060, 7091, 7121, 7152,
    7183, 7213, 7244, 7274, 7305, 7336, 7365, 7396, 7426, 7457, 7487, 7518,
    7549, 7579, 7610, 7640, 7671, 7702, 7730, 7761, 7791, 7822, 7852, 7883,
    7914, 7944, 7975, 8005, 8036, 8067, 8095, 8126, 8156, 8187, 8217, 8248,
    8279, 8309, 8340, 8370, 8401, 8432, 8460, 8491, 8521, 8552, 8582, 8613,
    8644, 8674, 8705, 8735, 8766, 8797, 8826, 8857, 8887, 8918, 8948, 8979,
    9010, 9040, 9071, 9101, 9132, 9163, 9191, 9222, 9252, 9283, 9313, 9344,
    9375, 9405, 9436, 9466, 9497, 9528, 9556, 9587, 9617, 9648, 9678, 9709,
    9740, 9770, 9801, 9831, 9862, 9893, 9921, 9952, 9982, 10013, 10043, 10074,
    10105, 10135, 10166, 10196, 10227, 10258, 10287, 10318, 10348, 10379,
    10409, 10440, 10471, 10501, 10532, 10562, 10593, 10624, 10652, 10683,
    10713, 10744, 10774, 10805, 10836, 10866, 10897, 10927, 10958
]

# Manually adjusted
FIRST_ROW_AFTER_BURNIN = 120
ANNOTATION_X_LOCATION = 3833
TITLE_FONTSIZE = 20
XLABEL_FONTSIZE = 20
YLABEL_PADDING = 100

# Generated from script:
# '''
# encoding_to_mn_df = pd.DataFrame({'Encoding': ENCODINGDB})
# for act in ['DHA-PPQ', 'ASAQ', 'AL']:
#   encoding_to_mn_df[act] = encoding_to_mn_df['Encoding'].apply(
#     lambda x: resistant_strength_calc(x, act)
#   )
# encoding_to_mn_dict = encoding_to_mn_df.set_index('Encoding').to_dict()
# '''
ENCODING_TO_MN_DICT = {
  'DHA-PPQ': {'KNY--C1x': '0-0',
    'KNY--C1X': '0-0',
    'KNY--C2x': '1-1',
    'KNY--C2X': '1-1',
    'KNY--Y1x': '1-1',
    'KNY--Y1X': '1-1',
    'KNY--Y2x': '2-2',
    'KNY--Y2X': '2-2',
    'KYY--C1x': '0-0',
    'KYY--C1X': '0-0',
    'KYY--C2x': '1-1',
    'KYY--C2X': '1-1',
    'KYY--Y1x': '1-1',
    'KYY--Y1X': '1-1',
    'KYY--Y2x': '2-2',
    'KYY--Y2X': '2-2',
    'KNF--C1x': '0-0',
    'KNF--C1X': '0-0',
    'KNF--C2x': '1-1',
    'KNF--C2X': '1-1',
    'KNF--Y1x': '1-1',
    'KNF--Y1X': '1-1',
    'KNF--Y2x': '2-2',
    'KNF--Y2X': '2-2',
    'KYF--C1x': '0-0',
    'KYF--C1X': '0-0',
    'KYF--C2x': '1-1',
    'KYF--C2X': '1-1',
    'KYF--Y1x': '1-1',
    'KYF--Y1X': '1-1',
    'KYF--Y2x': '2-2',
    'KYF--Y2X': '2-2',
    'KNYNYC1x': '0-0',
    'KNYNYC1X': '0-0',
    'KNYNYC2x': '1-1',
    'KNYNYC2X': '1-1',
    'KNYNYY1x': '1-1',
    'KNYNYY1X': '1-1',
    'KNYNYY2x': '2-2',
    'KNYNYY2X': '2-2',
    'KYYYYC1x': '0-0',
    'KYYYYC1X': '0-0',
    'KYYYYC2x': '1-1',
    'KYYYYC2X': '1-1',
    'KYYYYY1x': '1-1',
    'KYYYYY1X': '1-1',
    'KYYYYY2x': '2-2',
    'KYYYYY2X': '2-2',
    'KNFNFC1x': '0-0',
    'KNFNFC1X': '0-0',
    'KNFNFC2x': '1-1',
    'KNFNFC2X': '1-1',
    'KNFNFY1x': '1-1',
    'KNFNFY1X': '1-1',
    'KNFNFY2x': '2-2',
    'KNFNFY2X': '2-2',
    'KYFYFC1x': '0-0',
    'KYFYFC1X': '0-0',
    'KYFYFC2x': '1-1',
    'KYFYFC2X': '1-1',
    'KYFYFY1x': '1-1',
    'KYFYFY1X': '1-1',
    'KYFYFY2x': '2-2',
    'KYFYFY2X': '2-2',
    'TNY--C1x': '0-0',
    'TNY--C1X': '0-0',
    'TNY--C2x': '1-1',
    'TNY--C2X': '1-1',
    'TNY--Y1x': '1-1',
    'TNY--Y1X': '1-1',
    'TNY--Y2x': '2-2',
    'TNY--Y2X': '2-2',
    'TYY--C1x': '0-0',
    'TYY--C1X': '0-0',
    'TYY--C2x': '1-1',
    'TYY--C2X': '1-1',
    'TYY--Y1x': '1-1',
    'TYY--Y1X': '1-1',
    'TYY--Y2x': '2-2',
    'TYY--Y2X': '2-2',
    'TNF--C1x': '0-0',
    'TNF--C1X': '0-0',
    'TNF--C2x': '1-1',
    'TNF--C2X': '1-1',
    'TNF--Y1x': '1-1',
    'TNF--Y1X': '1-1',
    'TNF--Y2x': '2-2',
    'TNF--Y2X': '2-2',
    'TYF--C1x': '0-0',
    'TYF--C1X': '0-0',
    'TYF--C2x': '1-1',
    'TYF--C2X': '1-1',
    'TYF--Y1x': '1-1',
    'TYF--Y1X': '1-1',
    'TYF--Y2x': '2-2',
    'TYF--Y2X': '2-2',
    'TNYNYC1x': '0-0',
    'TNYNYC1X': '0-0',
    'TNYNYC2x': '1-1',
    'TNYNYC2X': '1-1',
    'TNYNYY1x': '1-1',
    'TNYNYY1X': '1-1',
    'TNYNYY2x': '2-2',
    'TNYNYY2X': '2-2',
    'TYYYYC1x': '0-0',
    'TYYYYC1X': '0-0',
    'TYYYYC2x': '1-1',
    'TYYYYC2X': '1-1',
    'TYYYYY1x': '1-1',
    'TYYYYY1X': '1-1',
    'TYYYYY2x': '2-2',
    'TYYYYY2X': '2-2',
    'TNFNFC1x': '0-0',
    'TNFNFC1X': '0-0',
    'TNFNFC2x': '1-1',
    'TNFNFC2X': '1-1',
    'TNFNFY1x': '1-1',
    'TNFNFY1X': '1-1',
    'TNFNFY2x': '2-2',
    'TNFNFY2X': '2-2',
    'TYFYFC1x': '0-0',
    'TYFYFC1X': '0-0',
    'TYFYFC2x': '1-1',
    'TYFYFC2X': '1-1',
    'TYFYFY1x': '1-1',
    'TYFYFY1X': '1-1',
    'TYFYFY2x': '2-2',
    'TYFYFY2X': '2-2'},
  'ASAQ': {'KNY--C1x': '1-1',
    'KNY--C1X': '1-1',
    'KNY--C2x': '1-1',
    'KNY--C2X': '1-1',
    'KNY--Y1x': '2-2',
    'KNY--Y1X': '2-2',
    'KNY--Y2x': '2-2',
    'KNY--Y2X': '2-2',
    'KYY--C1x': '1-2',
    'KYY--C1X': '1-2',
    'KYY--C2x': '1-2',
    'KYY--C2X': '1-2',
    'KYY--Y1x': '2-3',
    'KYY--Y1X': '2-3',
    'KYY--Y2x': '2-3',
    'KYY--Y2X': '2-3',
    'KNF--C1x': '0-0',
    'KNF--C1X': '0-0',
    'KNF--C2x': '0-0',
    'KNF--C2X': '0-0',
    'KNF--Y1x': '1-1',
    'KNF--Y1X': '1-1',
    'KNF--Y2x': '1-1',
    'KNF--Y2X': '1-1',
    'KYF--C1x': '1-1',
    'KYF--C1X': '1-1',
    'KYF--C2x': '1-1',
    'KYF--C2X': '1-1',
    'KYF--Y1x': '2-2',
    'KYF--Y1X': '2-2',
    'KYF--Y2x': '2-2',
    'KYF--Y2X': '2-2',
    'KNYNYC1x': '1-1',
    'KNYNYC1X': '1-1',
    'KNYNYC2x': '1-1',
    'KNYNYC2X': '1-1',
    'KNYNYY1x': '2-2',
    'KNYNYY1X': '2-2',
    'KNYNYY2x': '2-2',
    'KNYNYY2X': '2-2',
    'KYYYYC1x': '1-2',
    'KYYYYC1X': '1-2',
    'KYYYYC2x': '1-2',
    'KYYYYC2X': '1-2',
    'KYYYYY1x': '2-3',
    'KYYYYY1X': '2-3',
    'KYYYYY2x': '2-3',
    'KYYYYY2X': '2-3',
    'KNFNFC1x': '0-0',
    'KNFNFC1X': '0-0',
    'KNFNFC2x': '0-0',
    'KNFNFC2X': '0-0',
    'KNFNFY1x': '1-1',
    'KNFNFY1X': '1-1',
    'KNFNFY2x': '1-1',
    'KNFNFY2X': '1-1',
    'KYFYFC1x': '1-1',
    'KYFYFC1X': '1-1',
    'KYFYFC2x': '1-1',
    'KYFYFC2X': '1-1',
    'KYFYFY1x': '2-2',
    'KYFYFY1X': '2-2',
    'KYFYFY2x': '2-2',
    'KYFYFY2X': '2-2',
    'TNY--C1x': '1-2',
    'TNY--C1X': '1-2',
    'TNY--C2x': '1-2',
    'TNY--C2X': '1-2',
    'TNY--Y1x': '2-3',
    'TNY--Y1X': '2-3',
    'TNY--Y2x': '2-3',
    'TNY--Y2X': '2-3',
    'TYY--C1x': '1-3',
    'TYY--C1X': '1-3',
    'TYY--C2x': '1-3',
    'TYY--C2X': '1-3',
    'TYY--Y1x': '2-4',
    'TYY--Y1X': '2-4',
    'TYY--Y2x': '2-4',
    'TYY--Y2X': '2-4',
    'TNF--C1x': '1-1',
    'TNF--C1X': '1-1',
    'TNF--C2x': '1-1',
    'TNF--C2X': '1-1',
    'TNF--Y1x': '2-2',
    'TNF--Y1X': '2-2',
    'TNF--Y2x': '2-2',
    'TNF--Y2X': '2-2',
    'TYF--C1x': '1-2',
    'TYF--C1X': '1-2',
    'TYF--C2x': '1-2',
    'TYF--C2X': '1-2',
    'TYF--Y1x': '2-3',
    'TYF--Y1X': '2-3',
    'TYF--Y2x': '2-3',
    'TYF--Y2X': '2-3',
    'TNYNYC1x': '1-2',
    'TNYNYC1X': '1-2',
    'TNYNYC2x': '1-2',
    'TNYNYC2X': '1-2',
    'TNYNYY1x': '2-3',
    'TNYNYY1X': '2-3',
    'TNYNYY2x': '2-3',
    'TNYNYY2X': '2-3',
    'TYYYYC1x': '1-3',
    'TYYYYC1X': '1-3',
    'TYYYYC2x': '1-3',
    'TYYYYC2X': '1-3',
    'TYYYYY1x': '2-4',
    'TYYYYY1X': '2-4',
    'TYYYYY2x': '2-4',
    'TYYYYY2X': '2-4',
    'TNFNFC1x': '1-1',
    'TNFNFC1X': '1-1',
    'TNFNFC2x': '1-1',
    'TNFNFC2X': '1-1',
    'TNFNFY1x': '2-2',
    'TNFNFY1X': '2-2',
    'TNFNFY2x': '2-2',
    'TNFNFY2X': '2-2',
    'TYFYFC1x': '1-2',
    'TYFYFC1X': '1-2',
    'TYFYFC2x': '1-2',
    'TYFYFC2X': '1-2',
    'TYFYFY1x': '2-3',
    'TYFYFY1X': '2-3',
    'TYFYFY2x': '2-3',
    'TYFYFY2X': '2-3'},
  'AL': {'KNY--C1x': '1-2',
    'KNY--C1X': '1-2',
    'KNY--C2x': '1-2',
    'KNY--C2X': '1-2',
    'KNY--Y1x': '2-3',
    'KNY--Y1X': '2-3',
    'KNY--Y2x': '2-3',
    'KNY--Y2X': '2-3',
    'KYY--C1x': '1-1',
    'KYY--C1X': '1-1',
    'KYY--C2x': '1-1',
    'KYY--C2X': '1-1',
    'KYY--Y1x': '2-2',
    'KYY--Y1X': '2-2',
    'KYY--Y2x': '2-2',
    'KYY--Y2X': '2-2',
    'KNF--C1x': '1-3',
    'KNF--C1X': '1-3',
    'KNF--C2x': '1-3',
    'KNF--C2X': '1-3',
    'KNF--Y1x': '2-4',
    'KNF--Y1X': '2-4',
    'KNF--Y2x': '2-4',
    'KNF--Y2X': '2-4',
    'KYF--C1x': '1-2',
    'KYF--C1X': '1-2',
    'KYF--C2x': '1-2',
    'KYF--C2X': '1-2',
    'KYF--Y1x': '2-3',
    'KYF--Y1X': '2-3',
    'KYF--Y2x': '2-3',
    'KYF--Y2X': '2-3',
    'KNYNYC1x': '1-2',
    'KNYNYC1X': '1-2',
    'KNYNYC2x': '1-2',
    'KNYNYC2X': '1-2',
    'KNYNYY1x': '2-3',
    'KNYNYY1X': '2-3',
    'KNYNYY2x': '2-3',
    'KNYNYY2X': '2-3',
    'KYYYYC1x': '1-1',
    'KYYYYC1X': '1-1',
    'KYYYYC2x': '1-1',
    'KYYYYC2X': '1-1',
    'KYYYYY1x': '2-2',
    'KYYYYY1X': '2-2',
    'KYYYYY2x': '2-2',
    'KYYYYY2X': '2-2',
    'KNFNFC1x': '1-3',
    'KNFNFC1X': '1-3',
    'KNFNFC2x': '1-3',
    'KNFNFC2X': '1-3',
    'KNFNFY1x': '2-4',
    'KNFNFY1X': '2-4',
    'KNFNFY2x': '2-4',
    'KNFNFY2X': '2-4',
    'KYFYFC1x': '1-2',
    'KYFYFC1X': '1-2',
    'KYFYFC2x': '1-2',
    'KYFYFC2X': '1-2',
    'KYFYFY1x': '2-3',
    'KYFYFY1X': '2-3',
    'KYFYFY2x': '2-3',
    'KYFYFY2X': '2-3',
    'TNY--C1x': '1-1',
    'TNY--C1X': '1-1',
    'TNY--C2x': '1-1',
    'TNY--C2X': '1-1',
    'TNY--Y1x': '2-2',
    'TNY--Y1X': '2-2',
    'TNY--Y2x': '2-2',
    'TNY--Y2X': '2-2',
    'TYY--C1x': '0-0',
    'TYY--C1X': '0-0',
    'TYY--C2x': '0-0',
    'TYY--C2X': '0-0',
    'TYY--Y1x': '1-1',
    'TYY--Y1X': '1-1',
    'TYY--Y2x': '1-1',
    'TYY--Y2X': '1-1',
    'TNF--C1x': '1-2',
    'TNF--C1X': '1-2',
    'TNF--C2x': '1-2',
    'TNF--C2X': '1-2',
    'TNF--Y1x': '2-3',
    'TNF--Y1X': '2-3',
    'TNF--Y2x': '2-3',
    'TNF--Y2X': '2-3',
    'TYF--C1x': '1-1',
    'TYF--C1X': '1-1',
    'TYF--C2x': '1-1',
    'TYF--C2X': '1-1',
    'TYF--Y1x': '2-2',
    'TYF--Y1X': '2-2',
    'TYF--Y2x': '2-2',
    'TYF--Y2X': '2-2',
    'TNYNYC1x': '1-1',
    'TNYNYC1X': '1-1',
    'TNYNYC2x': '1-1',
    'TNYNYC2X': '1-1',
    'TNYNYY1x': '2-2',
    'TNYNYY1X': '2-2',
    'TNYNYY2x': '2-2',
    'TNYNYY2X': '2-2',
    'TYYYYC1x': '0-0',
    'TYYYYC1X': '0-0',
    'TYYYYC2x': '0-0',
    'TYYYYC2X': '0-0',
    'TYYYYY1x': '1-1',
    'TYYYYY1X': '1-1',
    'TYYYYY2x': '1-1',
    'TYYYYY2X': '1-1',
    'TNFNFC1x': '1-2',
    'TNFNFC1X': '1-2',
    'TNFNFC2x': '1-2',
    'TNFNFC2X': '1-2',
    'TNFNFY1x': '2-3',
    'TNFNFY1X': '2-3',
    'TNFNFY2x': '2-3',
    'TNFNFY2X': '2-3',
    'TYFYFC1x': '1-1',
    'TYFYFC1X': '1-1',
    'TYFYFC2x': '1-1',
    'TYFYFC2X': '1-1',
    'TYFYFY1x': '2-2',
    'TYFYFY1X': '2-2',
    'TYFYFY2x': '2-2',
    'TYFYFY2X': '2-2'}
}

# Generated from script:
'''
all_targeted_5_types_set = set()
# type_1_triple
all_targeted_5_types_set.update([x for x in ENCODINGDB if re.match('TYY..Y2.', x)])
# type_2_triple
all_targeted_5_types_set.update([x for x in ENCODINGDB if re.match('KNF..Y2.', x)])
# DHA-PPQ double
all_targeted_5_types_set.update(
  encoding_to_mn_df[encoding_to_mn_df['DHA-PPQ']=='2-2']['Encoding'].values
)
# ASAQ full-res double
all_targeted_5_types_set.update(
  encoding_to_mn_df[encoding_to_mn_df['ASAQ']=='2-4']['Encoding'].values
)
# AL full-res double
all_targeted_5_types_set.update(
  encoding_to_mn_df[encoding_to_mn_df['AL']=='2-4']['Encoding'].values
)
'''
ALL_TARGETED_5_TYPES_SET = {
  'KNF--Y1X',
  'KNF--Y1x',
  'KNF--Y2X',
  'KNF--Y2x',
  'KNFNFY1X',
  'KNFNFY1x',
  'KNFNFY2X',
  'KNFNFY2x',
  'KNY--Y2X',
  'KNY--Y2x',
  'KNYNYY2X',
  'KNYNYY2x',
  'KYF--Y2X',
  'KYF--Y2x',
  'KYFYFY2X',
  'KYFYFY2x',
  'KYY--Y2X',
  'KYY--Y2x',
  'KYYYYY2X',
  'KYYYYY2x',
  'TNF--Y2X',
  'TNF--Y2x',
  'TNFNFY2X',
  'TNFNFY2x',
  'TNY--Y2X',
  'TNY--Y2x',
  'TNYNYY2X',
  'TNYNYY2x',
  'TYF--Y2X',
  'TYF--Y2x',
  'TYFYFY2X',
  'TYFYFY2x',
  'TYY--Y1X',
  'TYY--Y1x',
  'TYY--Y2X',
  'TYY--Y2x',
  'TYYYYY1X',
  'TYYYYY1x',
  'TYYYYY2X',
  'TYYYYY2x'
}
TARGETED_TYPE1_TRI_LIST = ['TYY--Y2x', 'TYY--Y2X', 'TYYYYY2x', 'TYYYYY2X']
TARGETED_TYPE2_TRI_LIST = ['KNF--Y2x', 'KNF--Y2X', 'KNFNFY2x', 'KNFNFY2X']
TARGETED_TYPE3_DP_DOUBLE_LIST = [
  'KNY--Y2x', 'KNY--Y2X', 'KYY--Y2x', 'KYY--Y2X', 'KNF--Y2x',
  'KNF--Y2X', 'KYF--Y2x', 'KYF--Y2X', 'KNYNYY2x', 'KNYNYY2X',
  'KYYYYY2x', 'KYYYYY2X', 'KNFNFY2x', 'KNFNFY2X', 'KYFYFY2x',
  'KYFYFY2X', 'TNY--Y2x', 'TNY--Y2X', 'TYY--Y2x', 'TYY--Y2X',
  'TNF--Y2x', 'TNF--Y2X', 'TYF--Y2x', 'TYF--Y2X', 'TNYNYY2x',
  'TNYNYY2X', 'TYYYYY2x', 'TYYYYY2X', 'TNFNFY2x', 'TNFNFY2X',
  'TYFYFY2x', 'TYFYFY2X'
]
TARGETED_TYPE4_ASAQ_FULLDOUBLE_LIST = [
  'TYY--Y1x', 'TYY--Y1X', 'TYY--Y2x', 'TYY--Y2X', 'TYYYYY1x',
  'TYYYYY1X', 'TYYYYY2x', 'TYYYYY2X'
]
TARGETED_TYPE5_AL_FULLDOUBLE_LIST = [
  'KNF--Y1x', 'KNF--Y1X', 'KNF--Y2x', 'KNF--Y2X', 'KNFNFY1x',
  'KNFNFY1X', 'KNFNFY2x', 'KNFNFY2X'
]

# For Fig D MFT colorings
fig_d_mft_most_dang_double_2_2_geno_index = [
    6,
    7,
    14,
    15,
    22,
    23,
    30,
    31,
    38,
    39,
    46,
    47,
    54,
    55,
    62,
    63,
    70,
    71,
    78,
    79,
    86,
    87,
    94,
    95,
    102,
    103,
    110,
    111,
    118,
    119,
    126,
    127
]

fig_d_mft_most_dang_double_2_4_geno_index = [
    20, 21, 22, 23, 52, 53, 54, 55, 76, 77, 78, 79, 108, 109, 110, 111
]

'''
# manually adjusted
FIVE_YR_CYCLING_SWITCH_DAYS = [
    0, 3653, # Burn-in
    5478,    # First Five-Year Period
    7303,    # Second Period
    9128,    # Third
    10953,   # Fourth / Last
    10958    # Residue due to 365-and-calendar difference
]
'''
